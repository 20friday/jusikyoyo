/**
 * 종목명 → 종목코드 매핑
 * 국내 주요 종목 위주. DB에 없는 종목은 주가 조회 스킵.
 */
export const STOCK_CODES: Record<string, string> = {
  // 반도체
  '삼성전자': '005930',
  '삼성전자우': '005935',
  'SK하이닉스': '000660',
  '한미반도체': '042700',
  '이오테크닉스': '039030',
  '리노공업': '058470',
  '솔브레인': '357780',
  '원익IPS': '240810',
  '주성엔지니어링': '036930',
  'DB하이텍': '000990',
  '하나마이크론': '067310',

  // 전자부품
  '삼성전기': '009150',
  'LG이노텍': '011070',
  '삼성SDI': '006400',

  // AI·소프트웨어·SI
  'NAVER': '035420',
  '네이버': '035420', // 옛/한글 표기 — 과거 글 주가 조회 폴백용
  '카카오': '035720',
  '삼성SDS': '018260',
  'LG CNS': '064400',
  '현대오토에버': '307950',
  '카카오뱅크': '323410',

  // 로봇
  '두산로보틱스': '454910',
  '레인보우로보틱스': '277810',
  '로보티즈': '108490',
  '로보스타': '090360',
  '에스피지': '058610',

  // 방산·조선
  '한화에어로스페이스': '012450',
  'HD현대중공업': '329180',
  'HD한국조선해양': '009540',
  '삼성중공업': '010140',
  '한국항공우주': '047810',
  'LIG디펜스앤에어로스페이스': '079550',
  'LIG넥스원': '079550', // 옛 이름 — 과거 글 주가 조회 폴백용

  // 자동차
  '현대차': '005380',
  '기아': '000270',
  '현대모비스': '012330',
  'HD현대': '267250',
  '두산': '000150',

  // 2차전지·소재
  'LG에너지솔루션': '373220',
  '에코프로비엠': '247540',
  '포스코퓨처엠': '003670',
  '엘앤에프': '066970',
  '에코프로': '086520',

  // 전력설비
  'LS ELECTRIC': '010120',
  '두산에너빌리티': '034020',
  'HD현대일렉트릭': '267260',

  // 바이오
  '한미약품': '128940',
  '한올바이오파마': '009420',
  '셀트리온': '068270',
  '유한양행': '000100',

  // 금융·기타
  'KB금융': '105560',
  '신한지주': '055550',
  'SK스퀘어': '402340',
  'LG전자': '066570',
  'LG디스플레이': '034220',
  'LG유플러스': '032640',
  'SK텔레콤': '017670',
  'POSCO홀딩스': '005490',

  // 유통·소비재
  '신세계': '004170',
  '현대백화점': '069960',
};

export function getStockCode(name: string): string | null {
  return STOCK_CODES[name] ?? null;
}

/**
 * 옛 이름·약칭 → 정식 표시명 매핑.
 * 사명 변경이나 약칭(KAI 등)으로 다르게 저장된 과거 글도
 * 화면에는 하나의 정식 이름으로 보이게 한다.
 */
export const RENAMED_STOCKS: Record<string, string> = {
  'LIG넥스원': 'LIG디펜스앤에어로스페이스', // 2026.4 주총서 사명 변경 (LIG D&A)
  'KAI': '한국항공우주',                    // 약칭 통일
  '네이버': 'NAVER',                        // 정식 종목명은 NAVER

  // 방송에서 자주 쓰는 약칭·영문·구명 → KRX 정식 종목명(public/stocks.json 기준)으로 통일.
  // 정식명이 아니면 랭킹·검색·종목상세에서 조용히 누락되므로, 발견하는 대로 여기에 등록해 자동 교정한다.
  // ※ scripts/check-stock-names.mjs 의 ALIASES 와 같은 목적 — 둘 다 최신으로 유지할 것.
  'APR': '에이피알',
  '에이피알뷰티': '에이피알',
  'LS일렉트릭': 'LS ELECTRIC',
  '하닉스': 'SK하이닉스',
  'SK하닉스': 'SK하이닉스',
  '삼전': '삼성전자',
  '한화에어로': '한화에어로스페이스',
  'SM': '에스엠',
  'JYP': 'JYP Ent.',
  'D&D파마텍': '디앤디파마텍',
};

/** 옛 이름이면 현재 이름으로 바꿔서 돌려준다. 아니면 그대로. */
export function canonicalStockName(name: string): string {
  return RENAMED_STOCKS[name] ?? name;
}
